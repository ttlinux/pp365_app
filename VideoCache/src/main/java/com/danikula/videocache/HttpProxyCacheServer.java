...
import com.danikula.videocache.M3u8.M3U8StringUtils;
import com.danikula.videocache.M3u8.M3u8Cache;
...
public class HttpProxyCacheServer {
    ...
    public static final ExecutorService socketProcessor = Executors.newFixedThreadPool(8);
    ...
    public HttpProxyCacheServer(Context context,String Savepath) {
        this(new Builder(context,Savepath).buildConfig());
    }
    ...
    private void waitForRequest() {
        try {
            while (!Thread.currentThread().isInterrupted()) {
                Log.e("isInterrupted","isInterrupted");
    ...
    private void processSocket(Socket socket) {
        try {
            ...
            if (pinger.isPingRequest(url)) {
                ...
                if(M3U8StringUtils.isM3U8(GetRequest.getRequestUrl()))
                {
                    if(!url.startsWith("http") && !url.startsWith("https"))
                    {
                        url=M3U8StringUtils.GetUrl(GetRequest.getRequestUrl())+url;
                    }
                }
    ...
    public static final class Builder {
        ...
        public Builder(Context context,String SavePath) {
            this.sourceInfoStorage = SourceInfoStorageFactory.newSourceInfoStorage(context);
            this.cacheRoot = new File(SavePath);
            this.diskUsage = new TotalSizeLruDiskUsage(DEFAULT_MAX_SIZE);
            this.fileNameGenerator = new Md5FileNameGenerator();
            this.headerInjector = new EmptyHeadersInjector();
        }
