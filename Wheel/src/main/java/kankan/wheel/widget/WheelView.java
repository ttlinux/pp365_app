...
public class WheelView extends View {
   ...
   private boolean isMoreWheelbackgound,noitemline=true;
	...
	boolean IsShowdefaultBackground=true;

	//取消阴影效果
	boolean CancelShadow=false;
	...
	public WheelView(Context context, AttributeSet attrs, int defStyle) {
		...
		IsShowdefaultBackground=typedArray.getBoolean(R.styleable.WheelView_ShowdefaultBackground,true);
		initData(context);
		typedArray.recycle();
	}
	...
	{
		CancelShadow=true;
		IsShowdefaultBackground=false;
	}
	public void SetMoreWheelbackgound()
	{
		isMoreWheelbackgound=true;
	}
	/**
...
//				View view=this.getItemView(currentItem);
//
//				if(view instanceof TextView)
//				{
//					TextView textView=(TextView) view;
//					Log.e("nnnn",textView.getText().toString());
...
//				}
	...
	{
//		itemsLayout.setBackgroundColor(0x66387423);
		Log.e("itemsLayout", currentItem + "---->"+itemsLayout.getChildCount());
		for (int i=0;i<itemsLayout.getChildCount();i++)
		{
			if(itemsLayout.getChildAt(i) instanceof TextView)
			{
				TextView view=(TextView)itemsLayout.getChildAt(i);
				if(Integer.valueOf(view.getTag()+"")==currentItem)
				{
					view.setTextColor(SelectColor);
				}
				else
				{
					view.setTextColor(UNSelectColor);
				}
			}
		}
	}
	...
	private void drawItems(Canvas canvas) {
		...
		Log.e("itemsLayout", itemsLayout.getChildCount() + "  " + currentItem);

//		itemsLayout.setBackground(new ColorDrawable(0x88D6551E));
	 ...
	 * @param canvas the canvas for drawing 中间的那个啥
	 */
		...
		switch (event.getAction()) {
		    ...
		    case MotionEvent.ACTION_UP:
		        if (!isScrollingPerformed) {
					...
					{
						onUserClickListener.OnClick(this,currentItem);
					}
...
//			Log.e("index",index+"");
	...
	{
		void OnClick(WheelView wheelView,int index);
	}
}
